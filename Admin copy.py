...
def crear_base_datos():
    ...
    # Importar estudiantes desde Excel si no existen
    ...
    try:
        ...
        for _, row in df.iterrows():
            try:
                ...
                # Separar nombre en componentes si el Excel ya viene separado, si no, dejar vacío
                ...
                if documento and apellido1 and nombre1 and grado:
                    cursor.execute(
                        """
                        INSERT OR IGNORE INTO estudiantes
                        (tipo_documento, documento, apellido1, apellido2, nombre1, nombre2, sexo, fecha_nacimiento,
...
def crear_tabla_config():
    ...
    # Nueva estructura: la configuración depende de grado + area + evaluacion
    cursor.execute(
        """
    ...
    try:
        ...
        # Crear índice único para evitar duplicados por combinación grado+area+evaluacion
        try:
            cursor.execute(
                "CREATE UNIQUE INDEX IF NOT EXISTS idx_config_unica ON config_examenes (grado, area, evaluacion)"
            )
        except Exception:
            pass
...
def registrar_inicio(
    ...
    cursor.execute(
        """
        INSERT INTO resultados (documento, nombre, grado, area, nota, estado_examen, hora_inicio, hora_fin, intento, evaluacion, curso)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """,
        (
            documento,
    ...
    try:
        ...
        if respuestas_list:
            with sqlite3.connect(DB_FILE) as conn2:
                cur2 = conn2.cursor()
                cur2.execute(
                    "SELECT intento, apellido1, apellido2, nombre1, nombre2, grado FROM resultados WHERE documento = ? AND area = ? AND estado_examen = 'FINALIZADO' ORDER BY id DESC LIMIT 1",
                    (documento, area),
                )
                fila = cur2.fetchone()
                intento_val = fila[0] if fila else None
                apellido1_val = fila[1] if fila and len(fila) > 1 else ""
                apellido2_val = fila[2] if fila and len(fila) > 2 else ""
                nombre1_val = fila[3] if fila and len(fila) > 3 else ""
                nombre2_val = fila[4] if fila and len(fila) > 4 else ""
                grado_val = fila[5] if fila and len(fila) > 5 else None
                ...
                for r in respuestas_list:
                    ...
                    if not exists:
                        cur2.execute(
                            "INSERT OR IGNORE INTO respuestas_estudiantes (documento, apellido1, apellido2, nombre1, nombre2, grado, area, intento, pregunta_id, enunciado, respuesta_seleccionada, respuesta_correcta, es_correcta) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                            (
                                documento,
                                apellido1_val,
                                apellido2_val,
...
def cargar_areas():
    """Carga las áreas únicas desde el archivo de preguntas (sin "General" ficticia)."""
    try:
        df = pd.read_excel(PREGUNTAS_FILE)
        df.columns = df.columns.str.strip().str.lower()
        if "area" in df.columns:
            # Normalizar, eliminar duplicados y NaN
            areas = (
                df["area"]
                .astype(str)
                .str.strip()
            ...
            areas = sorted([a for a in areas if a and a.lower() != "none"])
            print(f"[DEBUG] Áreas cargadas: {areas}")
            return areas if areas else []
        print(f"[DEBUG] Columna 'area' no existe en Excel")
        return []
    except Exception as e:
...
def cargar_areas_por_grado(grado):
    """Carga las áreas únicas para un grado específico, filtrando estrictamente por grado.

    Args:
        grado: El grado del estudiante (ej: '1', '2', '3', etc.)

    Returns:
        Lista de áreas únicas para ese grado, o [] si no hay coincidencias.
    """
    print(f"[DEBUG] cargar_areas_por_grado llamada con grado={repr(grado)}")
    try:
        df = pd.read_excel(PREGUNTAS_FILE)
        df.columns = df.columns.str.strip().str.lower()
        print(f"[DEBUG] Excel cargado: {len(df)} filas, columnas: {list(df.columns)}")

        # Validar que existan columnas necesarias
        if "area" not in df.columns or "grado" not in df.columns:
            print(f"[DEBUG] ❌ Columnas 'area' o 'grado' no existen en Excel")
            return []

        # Normalizar grado de entrada IDÉNTICO a cargar_preguntas_filtradas
        grado_val = str(grado).strip().lower()
        print(f"[DEBUG] Grado NORMALIZADO: '{grado_val}'")

        # Normalizar columna grado en el DataFrame de la misma forma
        # que se normaliza en el resto del código (ej: cargar_preguntas_filtradas)
        df["grado"] = df["grado"].apply(normalizar_grado)

        # Filtrar por grado
        print(
            f"[DEBUG] Valores únicos de grado en Excel: {df['grado'].unique().tolist()}"
        )
        df_filtrado = df[df["grado"] == grado_val]
        print(
            f"[DEBUG] Filas después de filtrar por grado='{grado_val}': {len(df_filtrado)}"
        )

        if df_filtrado.empty:
            print(f"[DEBUG] ❌ No hay preguntas para grado='{grado_val}'")
            print(f"[DEBUG] Valores en Excel: {df['grado'].unique().tolist()}")
            return []

        # Obtener áreas únicas para este grado
        areas = (
            df_filtrado["area"]
            .astype(str)
            .str.strip()
        ...
        areas = sorted([a for a in areas if a and a.lower() != "none"])
        print(f"[DEBUG] areas para grado='{grado_val}': {areas}")
        return areas if areas else []
...
def cargar_evaluaciones_por_grado_y_area(grado, area):
    """Carga las evaluaciones únicas para una combinación específica de grado y área.

    Args:
        grado: El grado del estudiante (será normalizado)
        area: El área/asignatura seleccionada (será normalizada)

    Returns:
        Lista de evaluaciones (strings) ordenadas alfabéticamente, o [] si no hay coincidencias.
    """
    print(
        f"[DEBUG] cargar_evaluaciones_por_grado_y_area llamada con grado={repr(grado)}, area={repr(area)}"
    )
    try:
        # Validar que grado y area no estén vacíos
        ...
        if (
            "grado" not in df.columns
            or "area" not in df.columns
            or "evaluacion" not in df.columns
        ):
            print(f"[DEBUG] ❌ Columnas faltantes en Excel")
            return []

        # Normalizar columnas con los mismos criterios que el resto del código
        df["grado"] = df["grado"].apply(normalizar_grado)
        df["area"] = df["area"].astype(str).str.strip().str.lower()
        df["evaluacion"] = df["evaluacion"].astype(str).str.strip()

        # Normalizar parámetros de entrada
        grado_norm = normalizar_grado(grado)
        area_norm = str(area).strip().lower()

        print(f"[DEBUG] Filtrando: grado=='{grado_norm}' AND area=='{area_norm}'")

        # Filtrar por grado Y área
        df_filtrado = df[(df["grado"] == grado_norm) & (df["area"] == area_norm)]

        print(f"[DEBUG] Filas después de filtrar: {len(df_filtrado)} de {len(df)}")

        if df_filtrado.empty:
            print(
                f"[DEBUG] ❌ No hay preguntas para grado='{grado_norm}' Y area='{area_norm}'"
            )
            return []

        # Extraer evaluaciones únicas
        evaluaciones = (
            df_filtrado["evaluacion"].astype(str).str.strip().dropna().unique().tolist()
        )

        # Limpiar evaluaciones inválidas
        evaluaciones = [
            str(e).strip()
        ...
        print(
            f"[DEBUG] evaluaciones para grado='{grado_norm}', area='{area_norm}': {evaluaciones}"
        )
        return evaluaciones
...
def exportar_reporte_completo(grado_sel, area_sel):
    ...
    try:
        ...
        for fila in filas:
            ...
            # Obtener grado y curso oficial desde estudiantes.xlsx si está disponible
            try:
                ...
                if estudiante is not None:
                    ...
                    if "curso" in estudiante.index:
                        curso_oficial = estudiante["curso"]
                    elif "Curso" in estudiante.index:
                        curso_oficial = estudiante["Curso"]
                ...
                if curso_oficial is not None and not pd.isna(curso_oficial):
...
def cargar_preguntas_filtradas(area, grado, evaluacion=None):
    """Carga preguntas filtradas por evaluación (si está disponible) o por area+grado.

    IMPORTANTE: Cuando se proporciona `evaluacion`, el filtrado es ÚNICAMENTE por ese
    identificador único. No se usan area ni grado como filtros secundarios.

    Args:
        area: Área/asignatura (usado solo si evaluacion es None)
        grado: Grado del estudiante (usado solo si evaluacion es None)
        evaluacion: Identificador único de evaluación (ej: "soc4_p1_exam1").
                   Si se proporciona, es el ÚNICO filtro usado.

    Returns:
        DataFrame con preguntas filtradas, o DataFrame vacío si no hay coincidencias.
    """
    print(f"\n[DEBUG] ========== CARGAR_PREGUNTAS_FILTRADAS ==========")
    print(
        f"[DEBUG] Parámetros recibidos: area={repr(area)}, grado={repr(grado)}, evaluacion={repr(evaluacion)}"
    )

    try:
        df = pd.read_excel(PREGUNTAS_FILE)
        print(f"[DEBUG] Excel cargado: {len(df)} filas, {len(df.columns)} columnas")
    except Exception as e:
    ...
    if "area" not in df.columns or "grado" not in df.columns:
        print(f"[DEBUG] ❌ Columnas 'area' o 'grado' no existen")
        return pd.DataFrame()
    ...
    # IMPORTANTE: Normalizar evaluacion siempre
    if "evaluacion" not in df.columns:
        df["evaluacion"] = None
    else:
        # Convertir a string, strip, lowercase, y reemplazar valores "vacíos"
        df["evaluacion"] = (
            df["evaluacion"].fillna("").astype(str).str.strip().str.lower()
        )
        df["evaluacion"] = df["evaluacion"].apply(
            lambda x: None if x in ["nan", "none", ""] else x
        )

    print(f"[DEBUG] Valores ÚNICOS EN EXCEL:")
    print(f"  - areas: {df['area'].unique().tolist()}")
    print(f"  - grados: {df['grado'].unique().tolist()}")
    print(f"  - evaluaciones: {df['evaluacion'].dropna().unique().tolist()}")

    # REGLA PRINCIPAL: Si evaluacion se proporciona y no es None, SOLO filtra por evaluacion
    if evaluacion is not None and evaluacion.strip():
        ...
        if evaluacion_norm not in ["nan", "none", ""]:
            ...
            print(f"[DEBUG] Aplicando filtro: (evaluacion == '{evaluacion_norm}')")

            df_filtrado = df[df["evaluacion"] == evaluacion_norm]

            if not df_filtrado.empty:
                print(f"  → Filas encontradas: {len(df_filtrado)}")
                print(f"[DEBUG] RETORNANDO {len(df_filtrado)} preguntas")
                print(f"[DEBUG] ===================================================\n")
                if "id_contexto" in df_filtrado.columns and "id" in df_filtrado.columns:
                    return df_filtrado.sort_values(
                        by=["id_contexto", "id"]
                ...
                # Mostrar qué evaluaciones existen
                evals_disponibles = df["evaluacion"].dropna().unique().tolist()
                print(f"[DEBUG] Evaluaciones disponibles: {evals_disponibles}")
                return pd.DataFrame()

    # FALLBACK: Si no hay evaluacion, usar filtros de area+grado
    print(f"[DEBUG] Sin evaluacion específica. Filtrando por area+grado...")

    # Normalizar parámetros de entrada
    area_val = str(area).strip().lower()
    grado_val = normalizar_grado(grado)

    print(f"[DEBUG] Parámetros NORMALIZADOS: area='{area_val}' | grado='{grado_val}'")
    print(
        f"[DEBUG] Aplicando filtro: (area == '{area_val}') AND (grado == '{grado_val}')"
    )

    df_filtrado = df[(df["area"] == area_val) & (df["grado"] == grado_val)]

    print(f"  → Filas después del filtro: {len(df_filtrado)} (antes había {len(df)})")

    if df_filtrado.empty:
        print(
            f"[DEBUG] ❌ NO hay preguntas para área='{area_val}' Y grado='{grado_val}'"
        )
        # Mostrar qué combinaciones existen
        combos = df.groupby(["area", "grado"]).size().reset_index(name="count")
        print(f"[DEBUG] Combinaciones área+grado disponibles en Excel:")
        for _, row in combos.iterrows():
            print(
                f"  - area='{row['area']}', grado='{row['grado']}' ({row['count']} preguntas)"
            )
        return pd.DataFrame()

    print(f"[DEBUG] RETORNANDO {len(df_filtrado)} preguntas")
    print(f"[DEBUG] IDs de preguntas: {df_filtrado.get('id', pd.Series([])).tolist()}")
    print(f"[DEBUG] ===================================================\n")

    # Ordenar si existen columnas de contexto
    if "id_contexto" in df_filtrado.columns and "id" in df_filtrado.columns:
        return df_filtrado.sort_values(by=["id_contexto", "id"]).reset_index(drop=True)
...
def cargar_config_examen(area, grado=None, evaluacion=None):
    ...
    se prioriza la búsqueda por la combinación `grado + area + evaluacion`.
    Retorna: (duracion_segundos, cantidad_preguntas, max_intentos, permitir_reintentos, examen_activo)
    """
    try:
        conn = sqlite3.connect(DB_FILE)
        cursor = conn.cursor()

        # Si se dispone de grado y evaluacion -> buscar por los tres campos
        if grado is not None and evaluacion is not None:
            cursor.execute(
                "SELECT duracion_segundos, cantidad_preguntas, COALESCE(max_intentos, 1), COALESCE(permitir_reintentos, 1), COALESCE(examen_activo, 0) FROM config_examenes WHERE grado = ? AND area = ? AND evaluacion = ?",
                (grado, area, evaluacion),
            )
        # Si sólo grado está disponible -> buscar por grado + area
        elif grado is not None:
            cursor.execute(
                "SELECT duracion_segundos, cantidad_preguntas, COALESCE(max_intentos, 1), COALESCE(permitir_reintentos, 1), COALESCE(examen_activo, 0) FROM config_examenes WHERE grado = ? AND area = ?",
                (grado, area),
            )
        else:
            # Compatibilidad: búsqueda sólo por área
            cursor.execute(
                "SELECT duracion_segundos, cantidad_preguntas, COALESCE(max_intentos, 1), COALESCE(permitir_reintentos, 1), COALESCE(examen_activo, 0) FROM config_examenes WHERE area = ?",
                (area,),
            )
...
def examen_esta_activo(area):
    """Devuelve True si la configuración del área tiene examen_activo == 1."""
    try:
        _, _, _, _, examen_activo = cargar_config_examen(area)
        return bool(examen_activo)
    except Exception:
        return False
...
def guardar_config_examen(
    ...
    """Guarda o actualiza configuración de examen usando la combinación única grado+area+evaluacion."""
    try:
        conn = sqlite3.connect(DB_FILE)
        cursor = conn.cursor()
        ...
        if existe:
            cursor.execute(
                "UPDATE config_examenes SET duracion_segundos = ?, cantidad_preguntas = ?, max_intentos = ?, permitir_reintentos = ?, examen_activo = ? WHERE grado = ? AND area = ? AND evaluacion = ?",
                (
                    duracion_segundos,
                    cantidad_preguntas,
                    max_intentos,
            ...
            cursor.execute(
                "INSERT INTO config_examenes (grado, area, evaluacion, duracion_segundos, cantidad_preguntas, max_intentos, permitir_reintentos, examen_activo) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    grado,
                    area,
                    evaluacion,
...
class ModuloDocente:
    ...
    def __init__(self, ventana, nombre):
        ...
        # Cargar áreas desde preguntas.xlsx según el grado seleccionado (mantener comportamiento)
        ...
        self.combo_filtro_grado.bind(
            "<<ComboboxSelected>>", lambda e: self._on_grado_selected()
        )
    ...
    def _llenar_combos_filtros(self):
        ...
        try:
            ...
            # Llenar Cursos
            cursor.execute(
                "SELECT DISTINCT curso FROM resultados WHERE estado_examen IN ('FINALIZADO','PRESENTADO') AND curso IS NOT NULL ORDER BY curso"
            )
            cursos = [r[0] for r in cursor.fetchall() if r[0] is not None]
            ...
            self.combo_filtro_area["values"] = ["Todos"] + areas
    ...
    def _llenar_combo_grados(self):
        try:
            ...
            cursos = sorted([str(c).strip().upper() for c in cursor.fetchall() if c])
    ...
    def _llenar_combo_areas_por_grado(self):
        ...
        try:
            ...
            try:
                ...
                pass
            # Actualizar evaluaciones disponibles para la combinación grado+area
            try:
                self._llenar_combo_evaluaciones_por_grado_area()
            except Exception:
                pass
        except Exception:
            pass
    ...
    def _llenar_combo_evaluaciones_por_grado_area(self, grado=None, area=None):
        ...
        try:
            ...
            # Leer evaluaciones desde el archivo de preguntas
            evaluaciones = []
            try:
                df = pd.read_excel(PREGUNTAS_FILE)
                df.columns = df.columns.str.strip().str.lower()
                ...
                # Validar que existan columnas necesarias
                if (
                    "evaluacion" not in df.columns
                    or "grado" not in df.columns
                    or "area" not in df.columns
                ):
                    print(f"[DEBUG] Columnas faltantes en Excel")
                    raise Exception("Columnas faltantes")

                # Normalizar todas las columnas de la misma manera que se hace en cargar_preguntas_filtradas
                df["grado"] = df["grado"].apply(normalizar_grado)
                df["area"] = df["area"].astype(str).str.strip().str.lower()
                df["evaluacion"] = df["evaluacion"].astype(str).str.strip()

                print(
                    f"[DEBUG] Excel cargado: {len(df)} filas. Grados únicos: {df['grado'].unique().tolist()}, Áreas únicas: {df['area'].unique().tolist()}"
                )

                # FILTRAR ESTRICTAMENTE: grado == grado AND area == area
                print(f"[DEBUG] Filtrando: grado=='{grado}' AND area=='{area}'")
                df_filtrado = df[(df["grado"] == grado) & (df["area"] == area)]
                print(
                    f"[DEBUG] Filas después de filtrar: {len(df_filtrado)} de {len(df)}"
                )
                ...
                if not df_filtrado.empty:
                    ...
                    print(f"[DEBUG] Evaluaciones encontradas: {evaluaciones}")

            except Exception as e:
                print(f"[DEBUG] Error al leer Excel de evaluaciones: {e}")
                evaluaciones = []
    ...
    def _on_grado_selected(self, event=None):
        ...
        try:
            ...
            pass
        try:
            # Resetear evaluaciones cuando cambia el grado
            self._llenar_combo_evaluaciones_por_grado_area()
        except Exception:
            pass
        try:
...
class ModuloEstudiante:
    ...
    def _iniciar_examen(self, area):
        ...
        # Si no hay evaluación del combo, obtenerla de config_examenes
        if not evaluacion:
            try:
                conn = sqlite3.connect(DB_FILE)
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT evaluacion FROM config_examenes WHERE grado = ? AND area = ? AND examen_activo = 1 LIMIT 1",
        ...
        duracion, cantidad, max_intentos, permitir_reintentos, examen_activo = (
            cargar_config_examen(area, self.grado, evaluacion)
        )
        ...
        try:
            # Obtener curso del estudiante desde estudiantes.xlsx
            curso = None
            try:
    ...
    def _mostrar_examen(
        ...
        if todas is None or not hasattr(todas, "empty"):
            ...
            messagebox.showwarning(
                ...
                f"Error interno: el archivo de preguntas no retornó datos válidos.\n"
        ...
        if todas.empty:
            ...
            try:
                # Intentar leer el Excel para mostrar ejemplos de áreas y grados disponibles
                df_all = pd.read_excel(PREGUNTAS_FILE)
                ...
                detalles_adicionales = (
                    "\n\nEjemplos disponibles en 'preguntas.xlsx':\n"
                ...
                detalles_adicionales = (
                    "\n\n(No se pudo leer 'preguntas.xlsx' para mostrar ejemplos)"
                )
            ...
            messagebox.showwarning(
                ...
                f"• Grado: {self.grado}\n\n"
                f"Verifica que:\n"
                f"• El archivo '{PREGUNTAS_FILE.name}' tenga preguntas para esta combinación\n"
                f"• Las columnas 'area' y 'grado' estén correctamente configuradas (formatos/valores)"
...
def ingresar():
    ...
    if str(documento).strip().lower() == "admin":
        ...
        if msa.authenticate():
            # Destruir la ventana de login y crear una nueva raíz para SuperAdmin
            ventana.destroy()
            admin_root = tk.Tk()
            admin_root.withdraw()
            msa.parent = admin_root
            msa.open_interface()
            try:
    ...
    ModuloEstudiante(ventana_est, documento, nombre, grado)
