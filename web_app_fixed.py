from flask import Flask, session, render_template, send_from_directory, jsonify, send_file, request
import os
...
# Roles agregados automaticamente: {'CUSTOM_NEW_ROLE_FOR_TESTING'}
# Roles agregados automaticamente: {'NFKD', 'DELETE'}
import Admin
...
@app.route("/swagger")
def swagger_ui():
    """Interfaz visual Swagger UI para la API."""
...
@app.get("/api/resultados/panel")
def api_resultados_panel():
    """Panel consolidado del modulo Resultados."""
    try:
        ...
        return _api_error(exc, status=500)
    return _api_ok(**payload)
@app.get("/api/v1/resultados/detalle")
@app.get("/api/resultados/detalle")
def api_resultados_detalle():
    documento = request.args.get("documento")
    if not str(documento or "").strip():
        return _api_error("documento_requerido", status=400)
    try:
        payload = core_resultados_web.detalle_resultado(
            documento=documento,
            area=request.args.get("area"),
            intento=request.args.get("intento"),
        )
